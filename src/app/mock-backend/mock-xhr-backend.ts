...
export class MockXHRBackend implements HttpBackend {
			...
			switch (request.method) {
				case 'GET':
					if (request.urlWithParams.indexOf('mediaitems?medium=') >= 0 || request.url === 'mediaitems') {
						...
						const mediaItems = this.mediaItems.filter(i => i.id === idToFind);
						responseOptions = {
				...
				case 'POST':
					...
					break;
				case 'DELETE':
					this._deleteMediaItem(parseInt(request.url.split('/')[1], 10));
	...
	_getNewId(): number {
		if (this.mediaItems.length > 0) {
			return Math.max(...this.mediaItems.map(mediaItem => mediaItem.id)) + 1;
		} else {
			return 1;
		}
	}
